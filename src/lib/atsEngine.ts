...
import {
  MIN_KEYWORD_MATCH_RATIO,
  ATS_DOWNLOAD_THRESHOLD,
  MAX_KEYWORD_STUFFING,
  MIN_KEYWORDS_IN_SUMMARY,
  EMPLOYMENT_GAP_MONTHS,
} from './constants';

const COMPATIBLE_SYSTEMS = [
  'Workday', 'Taleo (Oracle)', 'Greenhouse', 'Lever', 'BambooHR',
  'iCIMS', 'SmartRecruiters', 'Jobvite', 'LinkedIn Easy Apply', 'Indeed Apply',
];
...
function scoreKeywords(
  ...
  kw: KeywordTier,
  r: ResumeData,
): { p1: ATSParameter; p2: ATSParameter; matched: string[]; missing: string[] } {
  ...
  let p2 = 0;
  p2 += summaryKwCount >= MIN_KEYWORDS_IN_SUMMARY ? MIN_KEYWORDS_IN_SUMMARY : Math.min(MIN_KEYWORDS_IN_SUMMARY, summaryKwCount);
  p2 += all.length === 0
    ? 5
    : Math.min(5, Math.round((skillsKwCount / Math.max(1, all.length)) * 5));
  p2 += expKwCount >= 5 ? 5 : Math.min(5, expKwCount);

  const stuffed = all.filter((k) => countOccurrences(text, k) > MAX_KEYWORD_STUFFING);
  ...
  return {
    p1: {
      ...
      issues: missing.length
        ? [`Missing ${missing.length} keywords: ${missing.slice(0, 5).join(', ')}${missing.length > 5 ? '…' : ''}`]
        : [],
      suggestions: missing.length
        ? ['Add missing keywords naturally into your summary, skills, and experience.']
        : [],
    },
    p2: {
      id: 'placement',
      ...
      max: 15,
      issues: [
        summaryKwCount < MIN_KEYWORDS_IN_SUMMARY
          ? `Only ${summaryKwCount} keywords in summary (need ${MIN_KEYWORDS_IN_SUMMARY}+)`
          : '',
        stuffed.length ? `Keyword stuffing detected: ${stuffed.join(', ')}` : '',
      ].filter(Boolean),
      suggestions:
        summaryKwCount < MIN_KEYWORDS_IN_SUMMARY
          ? ['Rewrite summary to include more target keywords.']
          : [],
    },
    matched,
    missing,
...
function scoreExperience(r: ResumeData, criticalKw: string[]): ATSParameter {
  ...
  const quantifiedBullets = allBullets.filter(
    (b) => /\d/.test(b) && /(%|\$|\bk\b|\bm\b|\bb\b|hours|weeks|days|months|years|users|customers|clients|times|fold|x\b)/i.test(b),
  );
...
function scoreSkills(r: ResumeData, kw: KeywordTier): ATSParameter {
  ...
  if (total === 0 || matched / total >= MIN_KEYWORD_MATCH_RATIO) score += 1;
...
function scoreDates(r: ResumeData): ATSParameter {
  ...
  for (let i = 1; i < sorted.length; i++) {
    ...
    if (prevEnd && sorted[i].startDate && monthsBetween(prevEnd, sorted[i].startDate) > EMPLOYMENT_GAP_MONTHS) {
      hasGap = true;
      break;
    }
  }
  ...
  return {
    ...
    max: 5,
    issues,
    suggestions: hasGap ? ['Briefly explain gaps with sabbatical, education, or freelance.'] : [],
  };
}
...
export function scoreResume(r: ResumeData, kw: KeywordTier): ATSReport {
  ...
  const compatibility = COMPATIBLE_SYSTEMS.map((s, i) => ({
    system: s,
    ok: percent >= ATS_DOWNLOAD_THRESHOLD || (percent >= 85 && i < 8),
  }));

  return { total, max, percent, parameters: params, keywords: kw, matchedKeywords: matched, missingKeywords: missing, compatibility };
}
